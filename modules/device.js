device                                          	=   {
	...
	hsearchmodus									:	undefined,
	hsearchdispl									:	undefined,
	remsize											:	undefined,

	report											:	function(){
		let devicereport = 'Displayed ' + window.device.orientation + ' on a ' + window.device.os + ' ' + window.device.type ;
		console.log('%c' + devicereport ,'color: grey; font-size:14px; font-weight:500; font-family:sans-serif;');
	},
    set                                             :   function(){
        if (window.device.type						==	'mobile'	) {						// SPLIT UP IN (15-25REM) (25-35REM) (35-45REM)
			if (window.device.orientation			==	'portrait'	) {
				device.remsize 						=	32;
				device.hsearchdispl					=	'none';
				device.hsearchmodus					=	' small ';
			}
		...
		if (window.device.type						==	'tablet'	) {
			...
			if (window.device.orientation			==	'landscape'	) {
				device.remsize						=	24;
				device.hsearchdispl					=	'flex';
				device.hsearchmodus					=	' large ';
		...
		REMSIZE = device.remsize;
		$(':root').css('--h-s-f-display',device.hsearchdispl);
		$(':root').css('--h-s-modus',device.hsearchmodus);
		$(':root').css('font-size', device.remsize + 'px');
    }
}
...
else if 	(device.macos()) 			{	addClass('macos desktop')		} 
